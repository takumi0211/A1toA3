...
async function splitAndMergePdf(inputArrayBuffer) {
    // オーバーラップ
    const overlap = getOverlapPoints();

    // ... 残りのコード ...
} 

// オーバーラップ値を取得する関数（単位: ポイント）
function getOverlapPoints() {
    // ローカルストレージから値を取得（デフォルト10mm）
    const overlapMm = Number(localStorage.getItem('pdfOverlap') || '10');
    // mmをポイントに変換（1mm = 72/25.4 ≈ 2.834645669291339 points）
    return (overlapMm / 25.4) * 72;
} 
